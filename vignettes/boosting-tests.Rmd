...
* Should try rerunning this once all the data issues have been sorted out (weird spike and drop possibly due to daylight savings)
...
Plotting our new crazy model's RMSE against the previous models shows that it outperforms all the others. However, this performance is based on the training and validation data set. We still need to assess how each model performs on the test data set.
...
Amazingly the model that uses all the lags again performs best (model 9). As expected, performance for all models is slightly worse on the test set, but the relative model performance remains similar.
