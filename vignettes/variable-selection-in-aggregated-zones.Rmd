...
Here we test variable selection methods for aggregated zones TOTAL and MASS against a baseline model. The variable selection methods include L1 and L2 penalisation within a linear booster. The baseline model will simply be the average of all weather stations that fall within an aggregated zone.
...
if (file.exists("./cache/variable-selection-in-aggregated-zones-no-lags.RData")) {
  ...
  system.time({
    ...
    cat("Fitting TOTAL with averaged variables. No penalty...\n")
    xgb_fit[["TOTAL"]][["AvgVars_NoLags_NoPenalty"]] <- smd_total_avg %>% 
    ...
    xgb_fit[["TOTAL"]][["AllVars_NoLags_L1Penalty"]] <- smd_total_all %>% 
      filter(Year %in% 2009:2015) %>% 
      select(Demand, Hour, DoY, DoW, Holiday_flag, contains("DryBulb"), 
             contains("DewPnt")) %>%
      train(Demand ~ . ,
    ...
    xgb_fit[["TOTAL"]][["AllVars_NoLags_L2Penalty"]] <- smd_total_all %>% 
      filter(Year %in% 2009:2015) %>% 
      select(Demand, Hour, DoY, DoW, Holiday_flag, contains("DryBulb"), 
             contains("DewPnt")) %>%
      train(Demand ~ . ,
    ...
    cat("Fitting MASS with averaged variables. No penalty...\n")
    xgb_fit[["MASS"]][["AvgVars_NoLags_NoPenalty"]] <- smd_mass_avg %>% 
      filter(Year %in% 2009:2015) %>% 
      select(Demand, Hour, DoY, DoW, Holiday_flag, contains("DryBulb"), 
             contains("DewPnt")) %>%
      train(Demand ~ . ,
    ...
    xgb_fit[["MASS"]][["AllVars_NoLags_L1Penalty"]] <- smd_mass_all %>% 
      filter(Year %in% 2009:2015) %>% 
      select(Demand, Hour, DoY, DoW, Holiday_flag, contains("DryBulb"), 
             contains("DewPnt")) %>%
      train(Demand ~ . ,
    ...
    xgb_fit[["MASS"]][["AllVars_NoLags_L2Penalty"]] <- smd_mass_all %>% 
      filter(Year %in% 2009:2015) %>% 
      select(Demand, Hour, DoY, DoW, Holiday_flag, contains("DryBulb"), 
             contains("DewPnt")) %>%
      train(Demand ~ . ,
