...
fcst_bs_df_vars <- c("Simulation", "ts", "Date_seq", "DoY", "DoW", "Year", 
                     "Hour", "Period", "Zone", "Holiday", "Holiday_flag",
                     "Weekend", "Prediction")
...
```{r plot_combine_pred_resid, fig.height=12}
fcst_bs_df %>%
  filter(Simulation == p_sim) %>% 
...
quantile_fcst <- fcst_bs_df %>% 
  ...
  ungroup() %>% 
  # TODO: Make sure these dates and hours are correct after fixing everything
  # from start.
  mutate(Date = date(ts),
         Hour = as.numeric(hour(ts)),
         Date = if_else(Hour == 0, Date - days(1), Date),
         Hour = if_else(Hour == 0, 24, Hour))
```
...
```{r hist_and_quantile_plots, fig.height = 13, fig.width = 8}
for (iZ in all_zones) {
...
WriteXLS(excel_sheets,
         ExcelFileName = "./output/D1-These boosts are made forecasting.xls")
```
