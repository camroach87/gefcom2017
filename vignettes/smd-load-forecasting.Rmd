...
pred_df_vars <- c("ts", "Date", "DoY", "Year", "Hour", "Period", "Zone", 
                  "Holiday", "Holiday_flag", "Demand", "Demand_pred")
...
for (iZ in load_zones) {
  ...
  pred_df <- smd %>% 
    ...
    mutate_at(vars(starts_with("DryBulb"), starts_with("DewPnt")),
              scale, center = FALSE) %>% 
    data.frame(.,
               Demand_pred = predict(xgb_fit[[iZ]], .)) %>% 
...
pred_df <- smd_mass %>% 
  mutate_at(vars(starts_with("DryBulb"), starts_with("DewPnt")),
              scale, center = FALSE) %>% 
    data.frame(.,
               Demand_pred = predict(xgb_fit[["MASS"]], .)) %>% 
    select(one_of(pred_df_vars)) %>% 
    bind_rows(pred_df)
...
pred_df <- smd_total %>% 
  mutate_at(vars(starts_with("DryBulb"), starts_with("DewPnt")),
              scale, center = FALSE) %>% 
    data.frame(.,
               Demand_pred = predict(xgb_fit[["TOTAL"]], .)) %>% 
    select(one_of(pred_df_vars)) %>% 
    bind_rows(pred_df)
...
```{r residual_plots}
pred_df %>% 
  ggplot(aes(x = DoY, y = Demand_resid)) +
...
pred_df %>% 
  filter(Year == 2015) %>% 
  ggplot(aes(x = Hour, y = Demand_resid)) +
  geom_point(shape = 21, alpha = 0.15) +
  geom_smooth(se = F) +
  facet_wrap(~Zone, scales = "free_y") +
...
pred_df %>% 
  filter(Year == 2015) %>% 
  ggplot(aes(x = Demand, y = Demand_resid)) +
  geom_point(shape = 21, alpha = 0.15) +
  geom_smooth(se = F) +
...
pred_df %>% 
  ...
  ggplot(aes(x=factor(Hour), y=Demand_resid)) + 
  geom_boxplot() +
  facet_wrap(~Zone, scales = "free") +
  xlab("Hour") +
...
Historical weather data will be bootstrapped in blocks that preserve intra-day weather correlation and seasonal weather correlation. Her we use a similar method to Hyndman and Shu (2010) (see bootstrapping doc for more info).

**TODO: carry out bootstrapping using function**
...
Based on the ACF plots produced earlier, it looks as though correlation only really holds for for two to three days in the residuals. We will use a simple block bootstrapping process, again similar to Hyndman and Shu (2010). All of the historical years of data will be broken into 
