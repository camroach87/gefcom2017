...
resid_df_vars <- c("ts", "Date", "DoY", "Year", "Hour", "Period", "Zone", 
...
###       there are some minor differences though so be careful
### Inputs: x, scale_vars
### Use: weather_bs %>%
###          group_by(Zone) %>% 
###          do(predict_zone(.)) %>% 
...
# not interested in demand because underlying weather/calendar variables have 
# changed after bootstrapping
fcst_df_vars <- c("Simulation", "ts", "Date_seq", "DoY", "Year", "Hour",
...
```{r}
p_sim <- sample(unique(fcst_df$Simulation), 1)
fcst_df %>%
...
# So need to group by Zone and Simulation when bootstrapping
n_periods <- length(unique(x_resid$Period))
n_days <- as.numeric(max(x_resid$Date) - min(x_resid$Date))
n_blocks <- ceiling(n_days/block_length)

x_resid$block <- rep(1:n_blocks, each = n_periods * block_length)[1:dim(x_resid)[1]]
...
#x_resid <- filter(x_resid, block %in% sample(1:n_blocks, fcst_n_blocks))
#fcst_df$block <- rep(bs_block_samples, each = n_perios * block_length)[1:dim(x_fcst)[1]]

```



# Base forecast reconciliation
