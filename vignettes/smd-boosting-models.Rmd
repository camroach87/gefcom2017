...
system.time({
  xgb_fit[["xgbLinear_lags"]] <-  train_data %>% 
...
test_data$Demand_pred_xgb_lags <- predict(xgb_fit[["xgbLinear_lags"]], newdata = test_data)
...
test_data %>%
  ...
  add_trace(y = ~Demand_pred_xgb, name = "xgbLinear", type = "scatter",
            mode = "lines") %>% 
  add_trace(y = ~Demand_pred_xgb_lags, name = "xgbLinear_lags",
...
test_data %>% 
  mutate(se_lm = (Demand - Demand_pred_lm)^2,
         ...
         se_xgb_lags (Demand - Demand_pred_xgb_lags)^2) %>% 
  summarise(mse_lm = mean(se_lm)^0.5,
            mse_xgb = mean(se_xgb)^0.5,
            mse_xgb_lags = mean(se_xgb_lags)^0.5) %>%
  gather(Model, Score) %>% 
  datatable()
```
